...
// OUT (n),A
// The output port is selected with address (almost always FE) of the integer
// along with register A, the byte in register A is copied to the address where
// the bits in the byte are read the action taken depending on which bits are
// on/off OpCodes: 0xD3
