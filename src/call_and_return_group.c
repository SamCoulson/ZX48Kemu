...
void CALL(uint16_t *addrs, uint16_t *spAddrs, uint16_t *sp, uint16_t *pc)
{
    ...
    *spAddrs = *pc;
    ...
    // Set the pc to point to the address -1 to compensate the autoincrment on
    // next loop
    *pc = *addrs - 1;
}
...
// 0xDC
void CALLC(uint16_t *addrs, uint16_t *spAddrs, uint16_t *sp, uint16_t *pc,
           uint8_t *fReg)
{
    if (getBit(fReg, 0) == 0x01)
    {
        ...
        *spAddrs = *pc;
        ...
        // Set the pc to point to the address -1 to compensate the autoincrment
        // on next loop
        *pc = *addrs - 1;
    }
}

// CALL NZ - Really not sure why these work when expression evaluates to 0
// instead of 1 Call if Z flag is reset 0xC4
void CALLNZ(uint16_t *addrs, uint16_t *spAddrs, uint16_t *sp, uint16_t *pc,
            uint8_t *fReg)
{
...
// CALL NC,nn
// 0xD4
void CALLNC(uint16_t *addrs, uint16_t *spAddrs, uint16_t *sp, uint16_t *pc,
            uint8_t *fReg)
{
...
// CALL PO
// 0xE4
void CALLPO(uint16_t *addrs, uint16_t *spAddrs, uint16_t *sp, uint16_t *pc,
            uint8_t *fReg)
{
    /*
    if( getBit( fReg, 0 ) != 0x00 ){
            ...
            *spAddrs = *pc;
            ...
            // Set the pc to point to the address -1 to compensate the
    autoincrment on next loop *pc = *addrs-1;
    }*/
    printf("Not implemented\n");
}

// 0xEC
void CALLPE(uint16_t *addrs, uint16_t *spAddrs, uint16_t *sp, uint16_t *pc,
            uint8_t *fReg)
{
    // Parity even
    printf("Not implemented\n");
...
// 0xF4
void CALLP(uint16_t *addrs, uint16_t *spAddrs, uint16_t *sp, uint16_t *pc,
           uint8_t *fReg)
{
    // Sign positive
    printf("Not implemented\n");
...
// OpCodes: 0xCC
void CALLZ(uint16_t *addrs, uint16_t *spAddrs, uint16_t *sp, uint16_t *pc,
           uint8_t *fReg)
{
...
// 0xFC
void CALLM(uint16_t *addrs, uint16_t *spAddrs, uint16_t *sp, uint16_t *pc,
           uint8_t *fReg)
{
    // Sign Negative
    printf("Not implemented\n");
...
// RET NZ
// Return if Z flag is zero /* Completely got me, surely non-zero means anything
// but 0 */ OpCodes: 0xC0
