...
int main() {
  ...
  // NOTE: no point in outputting every instruction that has been executed and
  // will be executed as you can't read them what I need is to be able to pause
  // at will or on a certain address or condition so why not have say -10 and
  // +10 instructions from the current instuction being executed shown in a list
  // all I would need do is decode or translate 20 instuctions, I could then
  // step the instructions and havbe then decoded as they flow into view.
  printf("about to run\n");
...
void doInterrupt() {
  ...
  //			// When the IFF is reset the CPU cannot accpet the
  // interrupt.
  //			// IFF1 actually hold the state and IFF2 is used as a
  // temporary storage space
  //			// for IFF1 state.
  //			// EI enables ( Sets IFF1 to enabled )
  //			// DI disables ( Sets IFF2 to disabled )
  //
  //			// Emulate ULA interrupting and reading video memory
  // only if INT enabled 			if( *reg->iff1 != 0 ){
  // readKeys();
  //
  //			}
  //
  ...
  //
  //
  //			// Emulate an NMI( Non-maskable interrupt ), is always
  // honered
  //			// independent of IFF (interrupt enable flip-flop)
  // status.  Always resets to 0066h
  //			// After an NMI on IFF1 is reset, IFF2 preserves the
  // state as part of being able to restore
  //			// the CPU state before execution of NMI routine.
  //			// when LD A,I or LD A,R or RETN occurs the state of
  // IFF2 is copied to
  ...
  //
  //			// After accepting a maskable interrupt both IFFs are
  // reset 			*reg->iff1 = 0;
  // *reg->iff2 = 0;
  //		}
  //		else
  ...
  // HALT - May also need to be implemented as it can be software generated, it
  // issues some NOP's while waiting for a NMI or interrupt request ( only is
  // IIF is enabled ) R Register should be implemented, 7-bits are incremented
  // after each instruction fetch Computer memory becasue the capacitors holding
  // 1/0 state loose charge and so need to be refreshed
}
