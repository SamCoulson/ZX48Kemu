...
int main()
{
    ...
    // will be executed as you can't read them what I need is to be able to
    // pause at will or on a certain address or condition so why not have say
    // -10 and +10 instructions from the current instuction being executed shown
    // in a list all I would need do is decode or translate 20 instuctions, I
    // could then step the instructions and havbe then decoded as they flow into
    // view.
    printf("about to run\n");
...
void doInterrupt()
{
    ...
    // HALT - May also need to be implemented as it can be software generated,
    // it issues some NOP's while waiting for a NMI or interrupt request ( only
    // is IIF is enabled ) R Register should be implemented, 7-bits are
    // incremented after each instruction fetch Computer memory becasue the
    // capacitors holding 1/0 state loose charge and so need to be refreshed
}
