...
void ADD(uint8_t *aReg, uint8_t *val, uint8_t *fReg)
{
    ...
    uint8_t result =
        *aReg + *val; // Save the result will copy to A register at end
    ...
    (getBit(aReg, 7) && getBit(val, 7)) ? setBit(fReg, 0, 1)
                                        : setBit(fReg, 0, 0);
...
void OR(uint8_t *dstReg, uint8_t *srcVal, uint8_t *fReg)
{
    ...
    // P/V is set if parity is even
    if ((getBit(dstReg, 7) == getBit(srcVal, 7)) &&
        (getBit(&result, 7) != getBit(dstReg, 7)))
    {
        // Set to 1 overflow
        setBit(fReg, 2, 1);
    }
