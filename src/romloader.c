...
int loadROMFile(const char *filename) {
  ...
  if (romFile != 0) {
    ...
    // Get the streampointer position (should reveal how long the file is in
    // bytes)
    ...
    // Place the file postion pointer to beginning of file read to begin reading
    // in the data
...
unsigned int ROMFileSize() { return romFileSize; }

// Is supposed to dump the contents of ROm data showing binra, char and address
// info
